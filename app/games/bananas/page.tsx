  ...
  const onUp = useCallback((e?: React.MouseEvent | React.TouchEvent) => {
    ...
    if (power > 10) {
      ...
      bananas.current.push({
        x: mouseX.current,
        y: H - 180 - 30,
        vx: Math.cos(a) * power * 0.16,
        vy: Math.sin(a) * power * 0.16,
        angle: 0,
        active: true,
      });
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      if (drag.current) {
        ...
        ctx.beginPath();
        ctx.moveTo(handX, tipY - 30);
        ctx.lineTo(handX - dx, tipY - 30 - dy);
        ctx.stroke();
        ctx.setLineDash([]);
        const len = Math.min(Math.sqrt(dx * dx + dy * dy), 250);
        const a = Math.atan2(dy, dx);
        ...
        ctx.moveTo(handX, tipY - 30);
        ctx.lineTo(
          handX + Math.cos(a) * len * 0.4,
          tipY - 30 + Math.sin(a) * len * 0.4
        );
        ctx.stroke();
      }
